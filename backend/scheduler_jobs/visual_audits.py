# ruff: noqa
"""
scheduler_jobs.visual_audits — Visual / fee-visibility audit scheduled jobs.

**Phase 2 incremental domain split — batch #10.**

Owns the recurring visual-regression audits for monetization surfaces.
Currently scoped to the fee-visibility visual audits (hourly + nightly).
Future expansion target: theme-visibility audit + visual SSR diff jobs.
...
logger = logging.getLogger("scheduler_jobs.visual_audits")


async def scheduled_fee_visibility_visual_audit_hourly():
    """Hourly visual contract audit with safe guardrails and lock control."""
    job_id = "fee_visibility_visual_audit_hourly"
    lock_key = "fee_visibility_visual_audit_hourly_lock"
    lock_acquired = False
    try:
        safe_config = await _load_fee_visibility_visual_safe_config()
        precheck = await _fee_visibility_visual_safe_precheck(
            job_id=job_id,
            min_interval_minutes=int(safe_config.get("hourly_min_interval_minutes") or 60),
        ...
        max_runtime_seconds = int(safe_config.get("max_runtime_seconds") or 150)
        result = await asyncio.wait_for(
            _run_scheduled_fee_visibility_visual_audit(triggered_by="scheduler:hourly"),
        ...
        await _record_scheduler_heartbeat(
            job_id,
            heartbeat,
            f"status={status} severity={result.get('severity')} drift={result.get('drift_count')} missing={result.get('missing_count')} escalation_sent={result.get('escalation_sent')} fail_streak={result.get('consecutive_fail_count')} mode=hourly fallback={precheck.get('fallback_applied')} relaxed={precheck.get('maintenance_relax_active')} max_runtime_s={max_runtime_seconds} safe_reasons={','.join(precheck.get('safe_reasons') or []) or 'none'}",
        )
    except Exception as exc:
        logger.error(f"Hourly fee visibility visual audit failed: {exc}")
...
async def scheduled_fee_visibility_visual_audit_nightly():
    """Nightly visual contract audit with safe guardrails and lock control."""
    job_id = "fee_visibility_visual_audit_nightly"
    lock_key = "fee_visibility_visual_audit_nightly_lock"
    lock_acquired = False
    try:
        safe_config = await _load_fee_visibility_visual_safe_config()
        precheck = await _fee_visibility_visual_safe_precheck(
        ...
        max_runtime_seconds = int(safe_config.get("max_runtime_seconds") or 150)
        result = await asyncio.wait_for(
            _run_scheduled_fee_visibility_visual_audit(triggered_by="scheduler:nightly"),
            timeout=max_runtime_seconds,
        )
        status = str(result.get("overall_status") or "unknown")
        heartbeat = "healthy" if result.get("ok") else "warning"
        await _record_scheduler_heartbeat(
            job_id,
            heartbeat,
            f"status={status} severity={result.get('severity')} drift={result.get('drift_count')} missing={result.get('missing_count')} escalation_sent={result.get('escalation_sent')} fail_streak={result.get('consecutive_fail_count')} mode=nightly fallback={precheck.get('fallback_applied')} relaxed={precheck.get('maintenance_relax_active')} max_runtime_s={max_runtime_seconds} safe_reasons={','.join(precheck.get('safe_reasons') or []) or 'none'}",
        )
    except Exception as exc:
        logger.error(f"Nightly fee visibility visual audit failed: {exc}")
        await _record_scheduler_heartbeat(job_id, "error", str(exc)[:180])
    finally:
        if lock_acquired:
            try:
...
def _default_fee_visibility_visual_safe_config() -> Dict[str, Any]:
    return {
        "hourly_min_interval_minutes": int(os.environ.get("FEE_VISUAL_SAFE_HOURLY_MIN_INTERVAL_MINUTES", "60") or 60),
        "nightly_min_interval_minutes": int(os.environ.get("FEE_VISUAL_SAFE_NIGHTLY_MIN_INTERVAL_MINUTES", "720") or 720),
        "min_disk_mb": float(os.environ.get("FEE_VISUAL_SAFE_MIN_DISK_MB", "220") or 220),
        "hard_floor_mb": float(os.environ.get("FEE_VISUAL_SAFE_HARD_FLOOR_MB", "140") or 140),
        "max_load_avg": float(os.environ.get("FEE_VISUAL_SAFE_MAX_LOAD_AVG", "12") or 12),
        "max_runtime_seconds": int(os.environ.get("FEE_VISUAL_SAFE_MAX_RUNTIME_SECONDS", "150") or 150),
        "lock_ttl_seconds": int(os.environ.get("FEE_VISUAL_SAFE_LOCK_TTL_SECONDS", "900") or 900),
    }

async def _load_fee_visibility_visual_safe_config() -> Dict[str, Any]:
    from routes.db import db

    defaults = _default_fee_visibility_visual_safe_config()
    now = datetime.now(timezone.utc)
    now_iso = now.isoformat()

    flag_doc = await db.system_runtime_flags.find_one(
        {"key": "fee_visibility_visual_safe_config"},
        {"_id": 0, "value": 1},
    ) or {}
    ...
    if relax_until_dt and relax_until_dt <= now and previous_config:
        ...
        await db.system_runtime_flags.update_one(
            {"key": "fee_visibility_visual_safe_config"},
            {"$set": {"key": "fee_visibility_visual_safe_config", "value": rolled_back, "updated_at": now_iso}},
            upsert=True,
        )
...
async def _fee_visibility_visual_safe_precheck(
    *,
    job_id: str,
    ...
    latest_scheduler_run = await db.platform_fee_visibility_visual_audits.find_one(
    ...
    recent_scheduler_runs = await db.platform_fee_visibility_visual_audits.find(
        {"triggered_by": {"$regex": "^scheduler:"}},
    ...
    await db.system_runtime_flags.update_one(
        {"key": f"{job_id}_safe_runtime_state"},
        {
            "$set": {
                ...
                "maintenance_relax_active": bool(safe_config.get("maintenance_relax_active")),
                "config_snapshot": {
                    "hourly_min_interval_minutes": int(safe_config.get("hourly_min_interval_minutes") or 60),
...
async def _run_scheduled_fee_visibility_visual_audit(*, triggered_by: str) -> Dict[str, Any]:
    ...
    url = (
        "http://127.0.0.1:8001/api/admin/platform-health/fee-visibility-visual-audit/run"
    ...
    escalation = payload.get("escalation") if isinstance(payload, dict) else {}
    return {
        "ok": response.status_code == 200 and str((audit or {}).get("overall_status") or "") == "pass",
        "status_code": response.status_code,
        "payload": payload,
        "overall_status": str((audit or {}).get("overall_status") or "unknown"),
        "severity": str((audit or {}).get("severity") or "unknown"),
        "drift_count": int((audit or {}).get("drift_count") or 0),
        "missing_count": int((audit or {}).get("missing_count") or 0),
        "escalation_sent": int((escalation or {}).get("sent") or 0),
        "consecutive_fail_count": int((escalation or {}).get("consecutive_fail_count") or 0),
    }
