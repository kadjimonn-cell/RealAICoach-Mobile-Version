...
def main() -> None:
    ...
    payload = {
        ...
        "top_offenders": [o.__dict__ for o in offenders[:50]],
    ...
    json_path = MEMORY_DIR / "phase4_i18n_plan.json"
    json_path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")

    md_lines = [
        "# Phase 4 i18n Reduction Plan",
        "",
        f"- Current total literals: **{current_total}**",
        f"- Phase 4 budget: **{TARGET_BUDGET}**",
    ...
    md_lines.append("| File | Literals |")
    md_lines.append("|---|---:|")
    for item in payload["top_offenders"][:25]:
        md_lines.append(f"| `{item['path']}` | {item['count']} |")

    md_path = MEMORY_DIR / "phase4_i18n_plan.md"
    md_path.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
