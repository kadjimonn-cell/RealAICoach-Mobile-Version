...
def _assert_base_url() -> str:
    assert BASE_URL, "REACT_APP_BACKEND_URL is required for Feature 29 v2 E2E tests"
...
def test_feature29_v2_bootstrap_and_play_flow(free_session: requests.Session):
    base = _assert_base_url()

    bootstrap = free_session.get(f"{base}/api/podcasts/v2/bootstrap", params={"tz": "UTC"}, timeout=45)
    assert bootstrap.status_code == 200, f"v2 bootstrap failed: {bootstrap.text}"
    payload = bootstrap.json()

    assert payload.get("feature_id") == "watch-videos-my-podcasts"
    assert isinstance(payload.get("catalog"), list)
    assert "podcast_story_arc" in payload
    assert "season_binge_rails" in payload
...
def test_feature29_v2_daily_drop_continue_and_mark_listened(free_session: requests.Session):
    base = _assert_base_url()

    inbox = free_session.get(f"{base}/api/podcasts/v2/daily-drop-inbox", timeout=30)
    assert inbox.status_code == 200, f"v2 inbox failed: {inbox.text}"
    inbox_data = inbox.json()
    items = inbox_data.get("items") or []

    continue_listening = free_session.get(f"{base}/api/podcasts/v2/continue-listening", timeout=30)
    assert continue_listening.status_code == 200, f"continue-listening failed: {continue_listening.text}"
    cont_payload = continue_listening.json()
    ...
    mark = free_session.post(
        f"{base}/api/podcasts/v2/daily-drop-inbox/mark-listened",
...
def test_feature29_legacy_wrapper_is_retired(free_session: requests.Session):
    base = _assert_base_url()

    legacy = free_session.get(f"{base}/api/videos/podcasts/bootstrap", params={"tz": "UTC"}, timeout=45)
    assert legacy.status_code in {404, 410}, f"Expected retired wrapper status, got {legacy.status_code}"

    modern = free_session.get(f"{base}/api/podcasts/v2/bootstrap", params={"tz": "UTC"}, timeout=45)
    assert modern.status_code == 200


def test_feature29_admin_conversion_dashboard_contract(admin_session: requests.Session):
    base = _assert_base_url()

    resp = admin_session.get(
        f"{base}/api/podcasts/v2/admin/conversion-dashboard",
    ...
    assert "continuation_rate_pct" in data.get("kpis", {})
